define(async (req, exports, module) => {
  ...
  (function () {
    ...
    var spell = function dictionary(dict_store) {
      var dict = dict_store && typeof dict_store.get === "function" ? dict_store.get() : {},
        noop = function () {},
      ...
      function spell_edits(word, alphabetOverride) {
        ...
        for (i = 0; i < word.length - 1; i++) {
          // transposition
          edits.push(word.slice(0, i) + word.slice(i + 1, i + 2) + word.slice(i, i + 1) + word.slice(i + 2));
        }
        for (i = 0; i < word.length; i++) {
          // alteration
       ...
       * @return void
       */
      function spell_reset() {
        return spell_load({ reset: true });
      }
      ...
      function spell_load(corpus, opts) {
        ...
        if ("object" === typeof opts.corpus) {
          ...
          spell_train(opts.corpus);
        }
        if (opts.store) {
          spell_store(opts.after_store);
        }
      }
      ...
      function spell_add_word(word, opts) {
        ...
        dict[word] = dict.hasOwnProperty(word) ? dict[word] + opts.score : opts.score;
        if (opts.store) {
          spell_store(opts.done);
        }
      }
      ...
      function spell_suggest(word, alphabet) {
        ...
        if (!is_empty(candidates)) {
          return spell_order(candidates, min, max);
        }
