define("/js/questionsLogic.js", async (req, exports, module) => {
  "use strict";
  class QuestionLogic {
    ...
    // }
    static makeHead({ favicon, canvas = this.canvas1, title, done = false }) {
      this.done1 = done;
    ...
    static loopStrHTML(symbol, tag, name, id) {
    ...
    static loopStr(symbol, tag, name, endTag = tag, id) {
      if (!this.initiated) throw new Error("Not initiated");
      this.res[id] += tag;
      this.pos[id] += 1;
      while (
    ...
    static parse(str1, lineSpacing = 1, test = false) {
      // if (!this.initiated)
    ...
    static makeList(list, type = "ol", options = { type: "1", lineSpacing: 1, id: "" }) {
      if (!this.initiated) throw new Error("Not initiated");
      var listHTML = document.createElement("ol");
      var isDiv = false;
      if (options.id) {
    ...
    static parseLink(link, text, options = this.defaultOptions.link) {
      if (!this.initiated) throw new Error("Not initiated");
  ...
  QuestionLogic.res = [];
  QuestionLogic.pos = [];
  QuestionLogic.length1 = [];
  QuestionLogic.count = 0;
  QuestionLogic.unanswered = 0;
  QuestionLogic.done1 = false;
  QuestionLogic.assignment = false;
  QuestionLogic.initiated = false;
  QuestionLogic.canvas1 = "https://csd509j.instructure.com/";
  // var OSName = "Unknown";
  ...
  //   console.log(str1);
  // }
  QuestionLogic.write = (str1) => {
    console.log(str1);
  };
  QuestionLogic.i = 0;
  // const ;
  QuestionLogic.parseOptions = {
    spellCheck: true,
    spellWords: {
      "abrige": "abridge",
      "auther": "author",
